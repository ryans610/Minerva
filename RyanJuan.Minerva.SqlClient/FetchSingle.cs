...
namespace RyanJuan.Minerva.SqlClientHelper
{
    public static partial class MinervaSqlClient
    {
        ...
        public static T FetchSingle<T>(
            this SqlCommand command,
            params object[] parameters)
        {
            return s_core.FetchSingle<T>(
                command,
                parameters);
        }

        /// <summary>
        /// 非同步版本的 <see cref="FetchSingle{T}(SqlCommand, object[])"/>。
        /// 執行查詢，並傳回查詢所傳回的結果集中唯一一筆資料，如果沒有資料，則為預設值。
        /// 如果結果集中有多個項目，則這個方法會擲回例外狀況。
        /// <para>
        /// 會以 <see cref="SqlCommand.Parameters"/> 呼叫
        ...
        /// <exception cref="InvalidOperationException"></exception>
        /// <exception cref="ObjectDisposedException"></exception>
        public static Task<T> FetchSingleAsync<T>(
            this SqlCommand command,
            params object[] parameters)
        {
            return command.FetchSingleAsync<T>(
        ...
        public static Task<T> FetchSingleAsync<T>(
            ...
            params object[] parameters)
        {
            return s_core.FetchSingleAsync<T>(
                command,
                cancellationToken,
                parameters);
        }
